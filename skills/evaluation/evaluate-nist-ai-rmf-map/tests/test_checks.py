"""Tests for evaluate-nist-ai-rmf-map."""
...
_SPEC = importlib.util.spec_from_file_location("nist_ai_rmf_map_checks", _SRC)
...
def _full_entry(*, coverage: float = 1.0) -> dict[str, object]:
    return {
        ...
        "evidence_uri": "https://wiki/system-cards/fraud-v3",
        "coverage": coverage,
        "resources": ["model://fraud-detector-v3"],
    }
...
class TestMalformedPayload:
    def test_entry_must_be_mapping(self):
        manifest = {"subcategories": {"MAP-1.1": "not-a-mapping"}}
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MAP-1.1")
...
class TestPartialPass:
    ...
    def test_low_coverage_is_fail(self):
        ...
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MAP-5.1")
        assert target.status == STATUS_FAIL
    ...
    def test_not_applicable_with_reason(self):
        manifest = {
            "subcategories": {
                "MAP-3.2": {
                    "not_applicable": True,
                    "not_applicable_reason": "Internal-only model with no monetary cost line",
                }
            }
        }
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MAP-3.2")
...
class TestSubcategoryFilter:
    def test_single_subcategory_returns_one_finding(self):
        ...
        assert findings[0].control_id == "MAP-1.1"
...
class TestOcsfProjection:
    def test_findings_render_as_compliance_findings(self):
        ...
        assert any(req.startswith("MAP:") for req in sample["compliance"]["requirements"])
