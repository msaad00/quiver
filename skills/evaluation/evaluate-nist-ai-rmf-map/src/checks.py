"""NIST AI RMF 1.0 — MAP function evaluation.

Implements 10 of the MAP function's ~18 documented subcategories as a
manifest-completeness + freshness audit. The manifest is YAML/JSON keyed
by subcategory ID (``MAP-1.1`` ... ``MAP-5.2``); typically each entry
points at a system / model card describing the AI system's intended
purpose, scientific basis, benefits, costs, and impacts.

This is NOT a substitute for the qualitative org-level assessment
NIST AI RMF requires. It validates that system cards exist, are current,
and cover the populations the framework asks for. The narrative the
program owner attaches to each card is what the assessment grades.
...
SKILL_NAME = "evaluate-nist-ai-rmf-map"
BENCHMARK_NAME = "NIST AI RMF 1.0 — MAP"
PROVIDER_NAME = "Multi"
FUNCTION = "MAP"
OUTPUT_FORMATS = ("native", "ocsf")
MANIFEST_ENV = "NIST_AI_RMF_MAP_MANIFEST"
...
# Curated subset of NIST AI RMF 1.0 MAP subcategories — 10 of ~18.
# Source: NIST AI RMF 1.0 Core (Section 5.2, table of subcategories).
IMPLEMENTED_SUBCATEGORIES: tuple[tuple[str, str, str, str], ...] = (
    ("MAP-1.1", "Intended purposes documented", "context", "HIGH"),
    ("MAP-1.2", "AI categorized per intended use", "context", "HIGH"),
    ("MAP-1.3", "AI capabilities + limitations + assumptions documented", "context", "HIGH"),
    ("MAP-2.1", "System task + AI capability documented", "system_card", "MEDIUM"),
    ("MAP-2.2", "Information about scientific / technical foundations", "system_card", "MEDIUM"),
    ("MAP-3.1", "System benefits documented", "impact", "MEDIUM"),
    ("MAP-3.2", "Costs documented", "impact", "MEDIUM"),
    ("MAP-4.1", "Approaches for mapping AI risks documented", "risk", "HIGH"),
    ("MAP-5.1", "Impacts on individuals + groups documented", "impact", "HIGH"),
    ("MAP-5.2", "Likelihood + magnitude of impacts assessed", "impact", "HIGH"),
)
