"""Tests for evaluate-nist-ai-rmf-measure."""
...
_SPEC = importlib.util.spec_from_file_location("nist_ai_rmf_measure_checks", _SRC)
...
def _full_entry(*, coverage: float = 1.0) -> dict[str, object]:
    return {
        ...
        "evidence_uri": "s3://airmf/measure-q2.json",
        "coverage": coverage,
        "resources": ["test-set://fraud-v3-q2"],
    }
...
class TestMalformedPayload:
    def test_entry_must_be_mapping(self):
        manifest = {"subcategories": {"MEASURE-1.1": ["wrong"]}}
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MEASURE-1.1")
...
class TestPartialPass:
    def test_documented_with_stale_review_is_partial(self):
        ...
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MEASURE-2.1")
    ...
    def test_low_coverage_is_fail(self):
        ...
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MEASURE-2.7")
        assert target.status == STATUS_FAIL
    ...
    def test_not_applicable_with_reason(self):
        manifest = {
            "subcategories": {
                "MEASURE-2.10": {
                    "not_applicable": True,
                    "not_applicable_reason": "Model handles no PII; privacy risk is structurally zero",
                }
            }
        }
        findings = run_benchmark(manifest, now=NOW)
        target = next(f for f in findings if f.control_id == "MEASURE-2.10")
