...
**80 shipped skill bundles** — atomic, deterministic, single-concern. Twelve are guarded write paths; the other 65 are read-only. Drop one into a pipeline, an agent, a Step Function, or a `python ... | python ...` one-liner.
...
| **Detect** | 33 | deterministic rules tagged with MITRE ATT&CK / ATLAS / OWASP | OCSF Detection Finding 2004 |
...
**Total: 80 shipped skills.**  Live counts and per-framework coverage in [`docs/COVERAGE_SNAPSHOT.md`](docs/COVERAGE_SNAPSHOT.md) (auto-generated, CI-gated).
